...
async function handleAvatarUpload(e: Event) {
  ...
  try {
    ...
    if (data.secure_url) {
      await auth.updateProfile({ avatar: data.secure_url })
    }
  } catch (err) {
      ...
      <div class="content-area">
        ...
        <div v-if="activeTab === 'seguranca'" class="panel">
          ...
          <div class="field">
            ...
            <div class="input-wrap"><i class="ti ti-lock"></i><input v-model="pwForm.atual" type="password" placeholder="••••••••" /></div>
          </div>
          <div class="field">
            <label>Nova senha</label>
            <div class="input-wrap"><i class="ti ti-lock"></i><input v-model="pwForm.nova" type="password" placeholder="Mínimo 6 caracteres" /></div>
          </div>
          <div class="field">
            <label>Confirmar nova senha</label>
            <div class="input-wrap"><i class="ti ti-lock-check"></i><input v-model="pwForm.confirmar" type="password" placeholder="Repita a senha" /></div>
          </div>
        ...
        <div v-if="activeTab === 'notificacoes'" class="panel">
          ...
          <div class="toggle-list">
            <div class="toggle-item">
              <div>
                <div class="toggle-label">Alertas críticos</div>
                <div class="toggle-desc">Sensor offline, umidade crítica, etc.</div>
              </div>
              <button class="toggle-btn" :class="{ on: notifications.email_alertas }" @click="notifications.email_alertas = !notifications.email_alertas">
            ...
            <div class="toggle-item">
              <div>
                ...
                <div class="toggle-desc">Resumo semanal da sua fazenda</div>
              </div>
              <button class="toggle-btn" :class="{ on: notifications.email_relatorios }" @click="notifications.email_relatorios = !notifications.email_relatorios">
                <span class="toggle-knob"></span>
              </button>
            </div>
            <div class="toggle-item">
              <div>
                <div class="toggle-label">Novidades do produto</div>
                <div class="toggle-desc">Novas funcionalidades e melhorias</div>
              </div>
              <button class="toggle-btn" :class="{ on: notifications.email_novidades }" @click="notifications.email_novidades = !notifications.email_novidades">
                <span class="toggle-knob"></span>
              </button>
            </div>
          </div>
          ...
          <div class="toggle-list">
            <div class="toggle-item">
              <div>
                <div class="toggle-label">Alertas em tempo real</div>
                <div class="toggle-desc">Receba alertas instantâneos no navegador</div>
              </div>
              <button class="toggle-btn" :class="{ on: notifications.push_alertas }" @click="notifications.push_alertas = !notifications.push_alertas">
                <span class="toggle-knob"></span>
              </button>
            </div>
            <div class="toggle-item">
              <div>
                <div class="toggle-label">Atualização de produção</div>
                <div class="toggle-desc">Notificação quando a produção mudar</div>
              </div>
              <button class="toggle-btn" :class="{ on: notifications.push_producao }" @click="notifications.push_producao = !notifications.push_producao">
                <span class="toggle-knob"></span>
              </button>
            </div>
          </div>
          ...
          <div class="theme-options">
            <button class="theme-option" :class="{ active: theme.dark }" @click="theme.dark || theme.toggle()">
              ...
              <div class="theme-option-label">
                ...
                <span v-if="theme.dark" class="active-badge">Ativo</span>
              </div>
            </button>
            <button class="theme-option" :class="{ active: !theme.dark }" @click="theme.dark && theme.toggle()">
...
.avatar-upload-btn.loading { background: var(--border); }
...
.btn-save:hover:not(:disabled) { background: var(--green2); }
...
.toggle-item { display: flex; justify-content: space-between; align-items: center; padding: 14px 16px; background: var(--surface2); border-radius: 12px; }
